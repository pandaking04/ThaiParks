...
export function SearchPage() {
  ...
  }, [parks, query, region, selectedActivities])
  ...
  const activeCount = selectedActivities.length
  const sortLabel = SORT_OPTIONS.find((o) => o.key === sortBy)?.label ?? ''

  return (
    <div className="fade">
      <div className="px-[30px] pt-[46px] pb-[30px] bg-gradient-to-b from-[#eef1e6] to-cream text-center">
        <div className="text-[11.5px] font-semibold tracking-[.2em] text-sage uppercase mb-3">
          ค้นพบอุทยานแห่งชาติไทย
        </div>
        <h1 className="font-heading font-bold text-[40px] leading-[1.1] text-forest max-w-[620px] mx-auto mb-3 tracking-tight">
          ออกไปสัมผัสธรรมชาติของเมืองไทย
        </h1>
        <p className="text-[15.5px] leading-[1.5] text-ink-muted max-w-[440px] mx-auto mb-6">
        ...
        <div className="flex items-center gap-2 bg-white border border-forest/14 rounded-full py-1.5 pl-5 pr-1.5 max-w-[540px] mx-auto shadow-[0_10px_26px_rgba(31,61,43,.09)] focus-within:shadow-[0_10px_30px_rgba(31,61,43,.16),0_0_0_3px_rgba(197,106,42,.18)] focus-within:border-clay transition-shadow">
          ...
          <input
            ...
            className="flex-1 border-none outline-none bg-transparent text-[15px] text-forest"
          />
          <button
            type="button"
        ...
        {geoStatus === 'denied' && (
          <p className="text-xs text-clay mt-2">ไม่ได้รับอนุญาตให้เข้าถึงตำแหน่ง กรุณาเปิดสิทธิ์ตำแหน่งในเบราว์เซอร์</p>
        )}
      </div>

      <div className="px-[30px] pt-2 pb-1 max-w-[1180px] mx-auto relative">
        {regions.length > 0 && (
        ...
        {filtersOpen && (
          <>
            <div className="fixed inset-0 z-45" onClick={() => setFiltersOpen(false)} />
            <div className="fade relative max-w-[640px] mx-auto mt-2.5 bg-white border border-black/10 rounded-2xl px-5 pt-4.5 pb-5 shadow-[0_16px_40px_rgba(31,61,43,.16)] z-46">
              ...
              <button
                ...
                className="mt-4.5 w-full text-center py-2.5 rounded-[11px] bg-forest text-cream-light font-semibold text-[13px]"
              >
                เสร็จสิ้น
              </button>
            </div>
      ...
      <div className="flex items-center justify-between px-[30px] pt-3.5 pb-4 max-w-[1180px] mx-auto relative">
        <button
          type="button"
        ...
        {sortOpen && (
          <>
            ...
            <div className="fade absolute top-[46px] left-[30px] bg-white border border-black/10 rounded-[13px] p-1.5 shadow-[0_14px_34px_rgba(31,61,43,.16)] z-46 min-w-[190px]">
              {SORT_OPTIONS.map((o) => (
                <button
                  key={o.key}
                  type="button"
      ...
      <div className="max-w-[1180px] mx-auto px-[30px] pb-[46px]">
